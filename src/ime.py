...
class Manager():
    ...
    def option_update(clss, prefs: Union[Prefs, SimpleNamespace], context: Context):
        ...
        # 该功能用于完全接管非IME文本输入，以便排除潜在的文本输入问题。
        # 大写锁定无法输入文本的这个问题，由 use_fix_direct_input_caps_lock 进行处理。
        # 此外目前没有非IME文本输入的问题，因此次功能目前是没有实际作用的。
        #
        # 而这个功能目前反而触发 BUG：
        # 微软拼音输入法在中文输入状态时，如果先输入0之外的数字，然后按字母键盘上的“.”键，
        # 会产生非IME输入“.”，再次按“.”键，会产生IME输入“。”。
        # 这个特性和本功能配合会出现问题。因为本功能会回放按键，相当于按下两次按键（但第一次会被屏蔽），
        # 因此正常只会产生一次非IME输入，但是微软IME自己记录是两次按键，于是会额外产生一次IME输入。
        # 因此按下字母键盘上的“.”键，结果是“.。”（Blender会在数字框自动将“。”转为“.”）。
        # 如果只按数字键盘上的“.”键不会触发该问题，开启大写锁定也不会触发问题。
        #
        # 综上述，该功能目前没有针对的对象，且会引发问题，因此停用。
